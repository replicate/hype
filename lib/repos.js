...
export async function getPythonRepos(filter = 'all') {
  ...
  const filteredRepos = repos.filter((repo) => repo.username && repo.username.trim() !== '');
...
async function getHuggingFaceRepoDescription(repo) {
  ...
  try {
    ...
    const description = firstHeadingMatch && firstHeadingMatch[1] ? firstHeadingMatch[1].trim() : '';
